...
5. **Document Type Label**: Small centered uppercase label below the header band — **"RECEIPT"** when fully paid, **"INVOICE"** when balance > 0. The document adapts rather than forcing the user to pick a type
6. **Status & Deadline**: Current order status (color-coded) + deadline date
7. **Priority Badge**: Only shown for URGENT or RUSH orders — red badge next to deadline
8. **Footer**: Truncated order ID (e.g., "Order #ORD-7A3F") plus an optional "Generated by StitchPad" attribution line (see Branding)
...
### Branding
- Business details limited to name and phone only (no email, no address)
- Scissors emoji (✂️) prefix on business name
- Phone emoji (📞) prefix on phone number
- **Optional product attribution footer:** when `User.businessName` is set (i.e. the tailor has personalised their brand), the receipt footer appends a small "Generated by StitchPad" line beneath the order ID. When `businessName` is null, the header already falls back to "StitchPad" and the attribution is suppressed to avoid repeating the brand name
...
- `documentTypeLabel: String` ("RECEIPT" when fully paid, "INVOICE" otherwise)
- `attribution: String?` ("Generated by StitchPad" when `User.businessName` is non-null, null otherwise)
