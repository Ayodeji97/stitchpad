...
class TriageGroupingTest {
    ...
    private val now = 1_700_000_000_000L // fixed reference — mid-afternoon in America/New_York
    ...
    fun emptyListReturnsEmptyMap() {
        val result = groupOrdersIntoTriage(emptyList(), now, zone)
    ...
    fun deliveredOrdersAreHidden() {
        ...
        val result = groupOrdersIntoTriage(listOf(delivered), now, zone)
        assertTrue(result.isEmpty())
    }
    ...
    fun readyOrdersGoToReadyForPickupEvenIfOverdue() {
        ...
        val result = groupOrdersIntoTriage(listOf(ready), now, zone)
    ...
    fun pastDeadlinePendingGoesToOverdue() {
        ...
        val result = groupOrdersIntoTriage(listOf(overdue), now, zone)
    ...
    fun inProgressWithFutureDeadlineGoesToInProgress() {
        ...
        val result = groupOrdersIntoTriage(listOf(ip), now, zone)
    ...
    fun pendingWithinSevenDaysGoesToDueThisWeek() {
        ...
        val result = groupOrdersIntoTriage(listOf(soon), now, zone)
    ...
    fun pendingExactlyAtSevenDaysGoesToDueThisWeek() {
        ...
        val result = groupOrdersIntoTriage(listOf(sevenDays), now, zone)
    ...
    fun pendingBeyondSevenDaysGoesToPending() {
        ...
        val result = groupOrdersIntoTriage(listOf(far), now, zone)
    ...
    fun pendingWithNoDeadlineGoesToPending() {
        ...
        val result = groupOrdersIntoTriage(listOf(noDl), now, zone)
    ...
    fun withinGroupSortedByDeadlineAscendingNullsLast() {
        ...
        val result = groupOrdersIntoTriage(listOf(a, b, c), now, zone)
    ...
    fun sameDeadlineSortedByCreatedAtDescending() {
        ...
        val result = groupOrdersIntoTriage(listOf(older, newer), now, zone)
    ...
    fun emptyGroupsAreOmitted() {
        ...
        val result = groupOrdersIntoTriage(listOf(pending), now, zone)
    ...
    fun resultIsInDisplayOrder() {
        ...
        val result = groupOrdersIntoTriage(listOf(pending, ready, inProg, overdue), now, zone)
        assertEquals(
